...
static bool tf_ambient_light_v3_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_AMBIENT_LIGHT_V3_CALLBACK_ILLUMINANCE: {
            ...
            TF_HalCommon *common = tf_hal_get_common(ambient_light_v3->tfp->hal);
            common->locked = true;
...
int tf_ambient_light_v3_create(TF_AmbientLightV3 *ambient_light_v3, const char *uid, TF_HalContext *hal) {
    ...
    //rc = tf_tfp_init(ambient_light_v3->tfp, numeric_uid, TF_AMBIENT_LIGHT_V3_DEVICE_IDENTIFIER, hal, port_id, inventory_index, tf_ambient_light_v3_callback_handler);
...
int tf_ambient_light_v3_get_illuminance(TF_AmbientLightV3 *ambient_light_v3, uint32_t *ret_illuminance) {
    ...
    uint32_t deadline = tf_hal_current_time_us(ambient_light_v3->tfp->hal) + tf_hal_get_common(ambient_light_v3->tfp->hal)->timeout;
...
int tf_ambient_light_v3_get_identity(TF_AmbientLightV3 *ambient_light_v3, char ret_uid[8], char ret_connected_uid[8], char *ret_position, uint8_t ret_hardware_version[3], uint8_t ret_firmware_version[3], uint16_t *ret_device_identifier) {
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&ambient_light_v3->tfp->spitfp->recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(ambient_light_v3->tfp->hal, ambient_light_v3->tfp->spitfp->port_id);
        }
