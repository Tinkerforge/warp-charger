...
\newcommand{\manualdate}{21.09.2026}
...
\begin{titlepage}
	\vspace*{-3.08cm}
	\colorbox{boxgray}{\makebox[\dimexpr\linewidth-2\fboxsep][c]{\includegraphics[width=0.6\textwidth]{./img4/logo}}}
	\vfill
	\begin{center}
		\Huge
		Betriebsanleitung\\\vspace{0.25cm}WARP4 Ladesäule\\\vspace{1cm}
		\large
		Version \manualversion\\\vspace{0.25cm}
		\manualdate
	\end{center}
	\vfill
	\begin{center}
...
\begin{multicols}{2}
	...
	Ausgelegt für die Montage von einer oder zwei WARP4 Charger Wallboxen sowie
	...
	Über die geteilte Rückwand können die Zuleitungen einfach erreicht und
	...
	vorgefertigten Anschlussleitungen geliefert. Der große Verteilerkasten
...
\vspace{-0.2cm}
	...
	Die Montage, erste Inbetriebnahme und Wartung der Ladestation dürfen nur von
	...
	Die Ladesäule besitzt eine geteilte Rückseite. Diese ist mit
	Verschlussriegeln ausgestattet, die mittels des mitgelieferten
	...
	Im Lieferumfang der Ladesäule befinden sich, abhängig davon, ob die Säule für eine
	Wallbox oder zwei Wallboxen vorgesehen ist:
	\begin{itemize}
	...
	Optional kann die Ladesäule mit einem Verteilergehäuse und Kabelanschlussset
...
\vspace{-0.1cm}
	...
	Befestigungsschrauben. Nach dem Aufstellen der Stele auf die Schrauben kann diese
	...
	linken Fußplatte der Ladesäule (siehe Foto). An dieser Schraube laufen alle
	Erdungsleitungen zusammen.

	\begin{center}
		\includegraphics[width=\linewidth]{./img4/earth_star.jpg}
	\end{center}

	Diese M6 Schraube muss extern mit Erde verbunden werden. Wird die Säule mit Verteilerbox
	bestellt, so liegt hierfür ein Erdungskabel 0,83m M6 Ringschuh auf Aderendhülse
	...
	anderweitig angeschlossen werden.

	\begin{center}
		\includegraphics[width=\linewidth]{./img4/earth_wb.jpg}
	\end{center}

	Von der Erdungsschraube im Fuß wird eine Erdungsleitung (1,5m 2x M6 Ringschuh) zur M6 Mutter gezogen,
	...
	Erdungsleitung zur Wallbox auf der Vorderseite und zur Wallbox auf der
	Rückseite (oberes Rückenteil) gezogen. An beiden Teilen ist dazu dann je eine M6
	Mutter vorhanden.
	...
	entsprechend konfektionierte Anschlussleitungen mitgeliefert.
    Diese sind 0,8m lang und auf einer Seite 12cm, auf der anderen 18cm abgemantelt.
    \par
    Es bietet sich an, die Anschlussleitung 	zuerst in der Wallbox zu installieren.
    Dazu müssen die Kabelverschraubungen an der Wallbox umgeschraubt werden. Die
    Kabelverschraubungen an der Unterseite (Auslieferungszustand) müssen entfernt
    und mit den Blindstopfen auf der Rückseite der Wallbox getauscht werden.
    Anschließend kann die Anschlussleitung mit der 18cm lang abgemantelten Seite
    rückseitig eingeführt und intern angeschlossen werden. Die Kabelverschraubung
    muss anschließend zugedreht werden.
    \par
    Bei der Variante mit Verteilerbox liegt zusätzlich ein RJ45-Patchkabel bei.
    Zum Einführen des Steckers wird der Dichtstopfen aus der M25-Kabelverschraubung
    genommen. Der Dichtstopfen muss seitlich bis zur Kabelbohrung geschlitzt,
    um das Kabel gelegt und wieder in die Verschraubung eingesetzt werden.
    Anschließend die Kabelverschraubung festziehen und das Patchkabel in der
    Wallbox anschließen.
	\par
	Anschließend kann die Wallbox mit den installierten Leitungen vor die Säule
	gehängt werden. Zur Befestigung werden die vier mitgelieferten M6x35mm
	Innensechskantschrauben verwendet.
    \par
    Sowohl das Anschlusskabel (12cm Seite) als auch das Patchkabel können anschließend
    durch die Kabelverschraubungen der Verteilerbox geführt werden und in dieser angeschlossen werden.

	\subsubsection{Verteilergehäuse}

	Das optionale Verteilergehäuse wird im Inneren der Säule unterhalb der Wallbox
	an den vorgesehenen Befestigungspunkten montiert (siehe Foto).
	Dazu den Deckel abnehmen und das Gehäuse mit den vier mitgelieferten
	M6x10mm Innensechskantschrauben befestigen. Nach dem Anschluss und der
	Prüfung der Installation den Deckel wieder schließen.

	\begin{center}
		\includegraphics[width=\linewidth]{./img4/box.jpg}
	\end{center}

	Im Verteilergehäuse befinden sich Klemmen vom Typ Contaclip ZRK 10/2A
	...
	angeschlossen werden. Dazu werden passende Patchkabel und ein
	...
	\begin{itemize}
		\item Anlage nur mit einem feuchten Tuch oder einem nicht aggressiven Reiniger abwischen (Anwendungshinweise des Herstellers beachten). Keine aggressiven Reinigungsmittel, Wachs oder Lösungsmittel verwenden.
		\item Teste das Reinigungsmittel immer erst an einer unauffälligen Stelle auf Verträglichkeit.
	\end{itemize}
	...
	\begin{minipage}{\linewidth}
		\begin{description}[leftmargin=!,labelwidth=\widthof{\textbf{WARP4 Charger}}]
			...
			\item[Material] Verzinkter Stahl in DB703 pulverbeschichtet
			\item[Materialstärke] \SI{2.0}{\milli\meter}
			\item[WARP4 Charger] 1 bis 2 montierbar
			\item[Abmessungen] ca. 390,6 × 1402,3 × \SI{154,5}{\milli\meter} (B/H/T ohne Fuß)
	...
	\begin{description}[leftmargin=!,labelwidth=\widthof{\textbf{Website}}]
        ...
        \item[Telefon] \texttt{05207 897300-0}
	\end{description}
	\end{minipage}
	...
	\begin{minipage}{0.40\textwidth}
		Die Ladesäule und die Verpackung sind bei Gebrauchsende ordnungsgemäß zu
	...
	\begin{tabular}{lll}
		...
		\midrule
		\manualdate & \manualversion & Initialversion              \\
		\bottomrule
	\end{tabular}
	...
	\label{appendix_base}
	\begin{center}
		\includegraphics[page=1,width=0.88\linewidth]{./img4/stand_drawings.pdf}
	\end{center}
	...
	\begin{center}
		\includegraphics[page=2,width=0.9\linewidth]{./img4/stand_drawings.pdf}
	\end{center}
