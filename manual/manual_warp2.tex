...
\begin{multicols*}{2}
	...
	\begin{description}
	 \item[Lastmanagement] Wenn das Lastmanagement aktiviert ist, erlaubt der Ladecontroller eine
	 Ladung nur, wenn diese durch einen Lastmanager freigegeben wurde. Der Lastmanager kann auf
	 diesem oder einem anderen WARP Charger im selben Netzwerk ausgeführt werden. Siehe \fullref{charge_manager}
	 für Details.
	 \item[Benutzerautorisierung] Wenn die Autorisierung aktiviert ist, müssen sich Benutzer über ein NFC-Tag
	 ausweisen, damit eine Ladung gestartet werden darf. Der Ladetracker ordnet die Ladung dann dem Benutzer zu.
	 Siehe \fullref{user_management} und \fullref{NFC} für Details.
	...
	\begin{description}
		...
		\item[Typ2-Ladekabel] Der Maximalstrom des Typ-2-Ladekabels. Wird durch einen festen Widerstand im Kabel vorgegeben.
		...
		\item[Externe Steuerung] Diese Ladestromgrenze wird duch eine externe Steuerung über die API, beispielsweise EVCC gesteuert.
	...
	\newpage
	\section{MQTT- und HTTP-Schnittstelle zur Fernsteuerung der Wallbox (Nur Smart / Pro)}
	...
	\hint{Wenn du die Zugangsdaten des HTTP-Anmeldung vergisst, kannst du nur nach einem Zurücksetzen auf den Werkszustand wieder darauf zugreifen.}
	...
	Auf der Ladecontroller-Unterseite \fullref{evse} kann die Option \enquote{Benutzerautorisierung} aktiviert werden.
	Wenn diese aktiv ist, muss ein NFC-Tag verwendet werden um eine Ladung zu starten.
	Wenn zusätzlich die \enquote{Tastereinstellung} auf \enquote{keine Aktion} konfiguriert wird,
	...
	% This hack allows to fit the last line of "Kontakt" on the same page.
	% Distribute the negative whitespace between the following sections.
	\vspace{-0.1cm}
	\section{Konformitätserklärung}
	Die EU-Konformitätserklärung zur Wallbox ist in einem gesonderten Dokument verfügbar.

	\vspace{-0.1cm} %see above
	\section{Entsorgung}
	...
	\begin{tabular}{lll}
		...
		06.12.2021 & 1.1.1   & Zuleitungsaderfarben            \\
		           &         & verbessert                      \\
		30.03.2022 & 2.0.0   & Aktualisiert für Firmware 2.0.0 \\
		\bottomrule
	\end{tabular}
	\newpage
	\pagestyle{empty}
