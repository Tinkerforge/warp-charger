...
\begin{multicols*}{2}
	...
	\subsubsection{LAN- / RJ45-Kabel anfertigen WARP 2.1}\label{ethernet}
	...
	\subsubsection{LAN- / RJ45-Kabel anfertigen - WARP 2.0}
	...
	\section{Erste Schritte (Nur Smart / Pro)}\label{setup}

	Bei der Basic Variante der Wallbox sind nach der elektrischen Installation
	keine weiteren Einstellmöglichkeiten vorhanden. Die nachfolgende
	...
	die Konfiguration über das Webinterface der Wallbox vorgenommen werden kann. 
	...
	Ist die Wallbox nun per WLAN (Accesspoint) oder LAN im Browser erreichbar,
	können alle weiteren Einstellungen durchgeführt werden.
	...
	Lastmanagements gewünscht sein (siehe \fullref{charge_manager}). Soll eine Ladefreigabe
	...
	\section{Webinterface (Nur Smart / Pro)}\label{webinterface}
	...
	Weitere Infos dazu erhälst du im Abschnitt \fullref{setup}.

	\subsection{Status (Startseite)}
	Wenn du auf das Webinterface der Wallbox zugreifst landest du auf der
	Startseite. Auf der rechten Seite befindet sich die Navigation, über die du
	zu weiteren Einstellungsmöglichkeiten kommst. Auf mobilen Endgeräten wird
	diese Navigation stattdessen versteckt unter einem Menü-Symbol oben rechts
	neben dem WARP Logo angezeigt. Hier musst du die Navigation durch Klick auf
	die drei Striche ausklappen.
	...
	Ab Firmwareversion 2.0.9 unterstützt die WARP Charger Smart und Pro  WireGuard. WireGuard
	ist eine Möglichkeit die Wallbox in ein virtuelles privates Netzwerk (VPN)
	...
	Lastmanagement zwischen den Wallboxen per WireGuard zusätzlich abgesichert werden.
	...
	zu löschen ist bei jedem angelegten Nutzer ein Verbotsschild vorhanden. Wird
	dieses aktiviert ist es dem Nutzer nicht mehr möglich sich anzumelden. Sein
	Passwort wird dann zusätzlich gelöscht, so dass dieses für eine erneute
	Anmeldung neu vergeben werden muss.

	Sollen mehrere Nutzer angelegt werden, so empfehlen wir es diese
	nacheinander anzulegen. Es muss kein Neustart der Wallbox nach jedem Nutzer
	durchgeführt werden. Es reicht ein Speichern und Neustart der Wallbox am
	Ende des Vorgangs.
	...
	durch Klicken auf eines der Tags kann dieses übernommen werden. Ein Neustart der
	Wallbox leert diese Liste. Sollen also mehrere Tags nacheinander hinzugefügt
	...
	werden. Wurden alle NFC Tags angelernt können die Tags gespeichert und die
	Wallbox einmal neugestartet werden.


	Alternativ können
	Tag-ID und -Typ manuell eingegeben werden. Dies ist zum Beispiel sinnvoll,
