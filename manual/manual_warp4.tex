...
\begin{multicols*}{2}
    ...
    \includegraphics[width=0.9\linewidth]{./img_warp3/resized/warp3_switches}
	...
	Es können auch größere Stecker genutzt werden, zum Beispiel
	werkzeuglose RJ45-Stecker oder solche mit einem LSA-Anschluss.
    ...
    WARP4 Charger können auf verschiedene Arten vom Netzbetreiber gesteuert werden.
    ...
    \subsection{Netzstützung nach OVE-Richtlinie \texorpdfstring{R\,37}{R 37} (Österreich)}\label{ove_r37}

	\hint{Diese Einstellungen dürfen nur von einer Elektrofachkraft durchgeführt
	und verändert werden.}

    In Österreich müssen Ladeeinrichtungen die Netzstützungsfunktionen nach
    OVE-Richtlinie R\,37 unterstützen. Der WARP4 Charger erfüllt diese
    Anforderungen. Die Konfiguration ist durch ein Elektriker-Passwort vor
    unbefugter Änderung geschützt und darf nur von einer Elektrofachkraft
    vorgenommen werden.

    \paragraph*{Aktivierung}
    Im Webinterface unter \texttt{System} $\rightarrow$ \texttt{Einstellungen} $\rightarrow$ \texttt{Land} die
    Einstellung \texttt{Österreich} auswählen und speichern. Anschließend
    erscheint der Menüpunkt \texttt{OVE R 37} unter \texttt{Energiemanagement}.

    \paragraph*{Konfiguration}
    Unter \texttt{OVE R 37} die Funktion aktivieren und speichern. Dazu ist das
    Elektriker-Passwort \texttt{Ele4223W} erforderlich. Die Einstellparameter
    sind im Auslieferungszustand bereits auf die von der Richtlinie geforderten
    Standardwerte eingestellt:
    \begin{itemize}
        \item Unterspannungs-Auslöseschwelle: 0,80\,pu (\SI{184}{\volt} bei \SI{230}{\volt} Nennspannung)
        \item Unterspannungs-Beobachtungszeit: \SI{3000}{\milli\second}
        \item Wiederzuschalt-Wartezeit: \SI{60}{\second}
        \item Ladestart-Verzögerung: \SI{0}{\second}
    \end{itemize}

    %\gfx{./img_warp4/web_ove_r37}
	...
	konfiguriert, ob ein \SI{16}{\ampere} Ladekabel (\SI{11}{\kilo\watt}) oder ein \SI{32}{\ampere} Ladekabel (\SI{22}{\kilo\watt})
    ...
    kabelgebundenen Netzwerk (IP-Adressbezug
    per DHCP). Die Wallbox kann dann entweder über die zugewiesene IP-Adresse (\url{http://[IP-der-Wallbox]}, z.\,B. \url{http://192.168.0.42})
    oder den Hostnamen der Wallbox (\url{http://[hostname]}, z.\,B.
    \url{http://warp4-ABC}) erreicht werden.

    Der Hostname der Wallbox ist identisch zur SSID des WLAN-Access-Points. Der Hostname findet sich
	...
	ebenfalls möglich. Ebenso die Unterstützung für dynamische Strompreise, das
	...
	\hint{Wir stellen hier nur die wesentlichen Elemente für die Ersteinrichtung
    ...
    Bestimmte Einträge (z.\,B. Lastmanagement) werden auf der Statusseite nur angezeigt, wenn die entsprechende
    ...
    Gegebenenfalls muss jetzt noch die Passphrase des gewählten Netzes eingetragen
	werden.
...
\includegraphics[width=.5\linewidth]{./img_warp4/badge-appstore.png}
    ...
    geben:
    \begin{itemize}
        \item Es liegt ein Fehler beim Ladekabel vor (Kurzschluss, verschmutzte / feuchte
    ...
    Auf der Recovery-Seite kann die Wallbox neu gestartet oder auf den Werkszustand zurückgesetzt (Factory Reset) werden und es können Firmware-Updates eingespielt, Debug-Reports
    ...
    \begin{description}[leftmargin=!,labelwidth=\widthof{\textbf{WARP4-CABLE-HARNESS}}]
     ...
     \item[WARP-T2-5M-32A] Typ-2-Stecker mit \SI{5}{\meter} Kabel, \SI{22}{\kilo\watt}/\SI{32}{\ampere}
     \item[WARP-T2-75M-16A] Typ-2-Stecker mit \SI{7,5}{\meter} Kabel, \SI{11}{\kilo\watt}/\SI{16}{\ampere}
     ...
     \item[WARP4-CASE-CP-PC] WARP4 Pro Gehäuse mit Edelstahlfrontblende DB703
