...
heating = Module("heating", T({'de': "Heizung", 'en': "Heating"}), "", T({'de': "Über das `heating`-Modul können Einstellungen für die Heizungssteuerung vorgenommen werden.", 'en': "The `heating` module allows settings for heating control to be configured."}), Version.WEM2, [
    ...
    Func("plan", FuncType.STATE, Elem.OBJECT(T({'de': "Der aktuelle Heizplan. Enthält für jeden Zeitslot der dynamischen Strompreise, ob dieser als günstig (erweiterter Betrieb) oder teuer (blockierender Betrieb) ausgewählt wurde. Der Plan wird unter Einhaltung der konfigurierten Mindesthaltezeit optimiert.", 'en': "The current heating plan. Contains for each time slot of the dynamic electricity prices whether it was selected as cheap (extended operation) or expensive (blocking operation). The plan is optimized to respect the configured minimum holding time."}), members={
        "cheap": Elem.ARRAY(T({'de': "Array von Wahrheitswerten. Jeder Eintrag entspricht einem Zeitslot der dynamischen Strompreise. `true` bedeutet, dass der Zeitslot für den erweiterten Betrieb ausgewählt wurde.", 'en': "Array of boolean values. Each entry corresponds to a time slot of the dynamic electricity prices. `true` means the time slot was selected for extended operation."}), member_type=EType.BOOL, is_var_length_array=True),
        "expensive": Elem.ARRAY(T({'de': "Array von Wahrheitswerten. Jeder Eintrag entspricht einem Zeitslot der dynamischen Strompreise. `true` bedeutet, dass der Zeitslot für den blockierenden Betrieb ausgewählt wurde.", 'en': "Array of boolean values. Each entry corresponds to a time slot of the dynamic electricity prices. `true` means the time slot was selected for blocking operation."}), member_type=EType.BOOL, is_var_length_array=True),
    })),
])
