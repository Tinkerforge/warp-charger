...
def tag(version):
    return [
        Elem.OBJECT(T({'de': "Ein autorisiertes NFC-Tag", 'en': "An authorized NFC tag"}), version=version, members={
            ...
            "tag_type": Elem.INT(T({'de': "Typ des Tags.", 'en': "Type of the tag."}), censored_in_debug_report=True, constants=[
            ...
            "tag_id": Elem.STRING(T({'de': "ID des Tags. Je nach Tag-Typ bis zu 10 Hex-Bytes, separiert durch ':'. z.B. `01:23:AB:3D`", 'en': "ID of the tag. Depending on tag type, up to 10 hex bytes separated by ':'. e.g. `01:23:AB:3D`"}), censored_in_debug_report=True),
        })
    ]
