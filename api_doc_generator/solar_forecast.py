...
solar_forecast = Module("solar_forecast", T({'de': "PV-Ertragsprognose", 'en': "PV Yield Forecast"}), "", T({'de': "Über das `solar_forecast`-Modul kann eine PV-Ertragsprognose konfiguriert und ausgelesen werden.", 'en': "The `solar_forecast` module allows configuration and reading of a PV yield forecast."}), Version.WARP2 | Version.WARP3 | Version.WARP4 | Version.WEMX, [
    ...
    Func("state", FuncType.STATE, Elem.OBJECT(T({'de': "Der Zustand der PV-Ertragsprognose.", 'en': "The state of the PV yield forecast."}), members={
        ...
        "rate_remaining": Elem.INT(T({'de': "Gibt an, wie viele API-Aufrufe im aktuellen Zeitfenster verbleiben.", 'en': "Indicates how many API calls remain in the current time window."})),
    ...
    Func("planes/X/forecast_update", FuncType.COMMAND, Elem.OBJECT(T({'de': "Aktualisiert die PV-Ertragsprognose einer PV-Fläche über die API. Funktioniert nur wenn die `source` auf Push (1) gesetzt ist. Falls die jeweilige PV-Fläche noch nicht aktiviert ist, wird sie automatisch aktiviert.", 'en': "Updates the PV yield forecast of a PV plane via the API. Only works when the `source` is set to Push (1). If the respective PV plane is not yet enabled, it will be automatically enabled."}), members={
        "first_date":  Elem.INT(T({'de': "Ein Unix-Timestamp in <strong>Minuten</strong>, der angibt, zu welchem Zeitpunkt die erste PV-Ertragsprognose im `forecast`-Array gilt.", 'en': "A Unix timestamp in <strong>minutes</strong> indicating the time for which the first PV yield forecast in the `forecast` array is valid."})),
        "resolution":  Elem.INT(T({'de': "Zeitliche Auflösung der PV-Ertragsprognose im `forecast`-Array.", 'en': "Time resolution of the PV yield forecast in the `forecast` array."}), constants=[
            Const(0, T({'de': "15 Minuten", 'en': "15 minutes"})),
            Const(1, T({'de': "60 Minuten", 'en': "60 minutes"})),
