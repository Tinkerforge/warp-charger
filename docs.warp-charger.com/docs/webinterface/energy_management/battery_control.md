...
:::note
    ...
    Speicher können von einem [WARP Charger](/docs/warp_charger/introduction) oder [WARP Energy Manager](/docs/warp_energy_manager/introduction) über das Netzwerk gesteuert werden.
    ...
    Die aktuell unterstützen Speicher bzw. Hybrid Wechselrichter sind in der [Liste der kompatiblen Geräte](/docs/compatible_devices/devices) zu finden. Sie unterstützen das Feature [Batteriesteuerung (BS)](/docs/compatible_devices/introduction#batteriesteuerung-bs).
...
Für jede Regel kann eine **Beschreibung** (Freitext) angelegt werden, sowie Bedingungen die auf die Parameter **Zeit**, den **State-of-Charge (SoC)** der Batterie, der aktuelle (dynamische) **Strompreis**, die **PV-Ertragsprognose**,
...
Bedingung oder ein SoC **größer** als 50% definiert werden kann. Es müssen nicht alle Parameter als Bedingung der Regel genutzt werden. Parameter können ignoriert werden.
...
Die Konfiguration der Regeln ist immer kunden- und anwendungsspezifisch. Nachfolgend geben wir einige Beispiele. Die jeweiligen Parameterwerte sind aber immer Abhängig von der jeweiligen PV-Anlage, des Speichers und den Verbräuchen.
...
<table>
<tr><td><b>Beschreibung</b></td><td><b>Zeit</b></td><td><b>SoC</b></td><td><b>Strompreis</b></td><td><b>PV-Ertrag</b></td><td><b>Strompreisplan</b></td><td><b>Schnellladung</b></td></tr>
...
Die zweite Regel ist dazu gedacht, dass wenn die erste Regel nicht greift, bei einem SoC unter 60% dennoch zu den günstigen Stunden bis 60% zu laden aber nur wenn der Strompreis immer noch unter 8 Cent liegt. Die dritte Regel ist dazu gedacht,
dass wenn die ersten beiden Regeln nicht greifen, bei einem SoC unter 30% dennoch zu den günstigen Stunden bis 30% zu laden. Hierbei wird der eigentliche Strompreis und der erwartete PV-Ertrag dann ignoriert. Die Regel ist aktiv wenn wir uns in den günstigen Stunden befinden.
...
Die Regel verbietet das Laden des Speichers am Vormittag, wenn der Speicher schon einen SoC von > 70% besitzt und ein großer PV-Ertrag zu erwarten ist. Damit wird dann morgens noch eingespeist zu Zeiten
an denen das Stromnetz den Strom noch benötigt und man noch eine Einspeisevergütung erhält. Wir behalten uns aber noch ~30% Akkukapazität übrig um zu den Zeiten in denen das Stromnetz den Strom nicht mehr benötigt (Strompreis kleiner als 0 Cent)
...
Es gibt gute Gründe um einen Speicher zu verbieten zu entladen. Als Beispiel macht es keinen Sinn den Speicher zu günstigen Strompreisen aus dem Netz zu laden und direkt danach bei "nicht teuren" Strompreisen zu entladen (Umwandlungsverluste etc.).
...
Die Regel dient dazu den Speicher nur zu entladen, wenn der Strompreis teuer ist (umgesetzt als Entladeverbot bei **nicht teuer**). Damit würde der Speicher genutzt um einen Strombezug während der teuren Stunden laut definierten Stromplan zu verhindern.
...
In diesem Fall (**1b**) würde der Speicher bis 80% SoC immer entladen, darunter aber nur in den teuren Stunden.
...
Auch diese Regel kann erweitert werden zum Beispiel mit einer Bedingung zum SoC, so dass nur ein Teil der Batteriekapazität zum Laden des Fahrzeugs verwendet wird.
...
dann wird der Speicher bis auf 100\% geladen (keine Beschränkung des Soc). Mit dieser Regel wird der Speicher im Winter bei sehr günstigen Strompreisen nachts vollgeladen.
...
Der Grund für die Entladeregeln ist, dass bei einem SoC kleiner 100\% der Speicher als Beispiel bis 80\% Soc geladen wird. Dann werden die Bedingung der Regel mit kleiner als 80\% Soc nicht mehr erfüllt. Der Speicher geht wieder in den "Normalmodus" und entlädt sich.
Bei 79\% SoC wird die Laderegel wieder aktiviert und lädt den Speicher wieder bis auf 80\% Soc. Das System schwingt also (Laden/Entladen). Um dieses Schwingen zu unterbinden wurden jeweils zu jeder Laderegel eine Entladeregel mit 1\% höherem SoC angelegt.
Somit ist gleichzeitig zum Laden in dem Zustand immer das Entladen verboten und der SoC bleibt unverändert.

Die Regel **Reserve halten** ist analog aufgebaut (Lade- und Entladeregel) und dient dazu einen SoC von 30\% als Reserve zu halten, der für die teuren Stunden genutzt wird um in diesen keinen Strom beziehen zu müssen. Die Regeln sorgen dafür, dass auf 30\% SoC
aufgeladen in den nicht teuren Stunden aufgeladen wird und dieser SoC auch gehalten wird (1\% höhere Entladen-blockieren-Regel). Befinden wir uns in einem Zeitraum, der laut Strompreisplan **teuer** ist, so greifen die Regeln nicht und der Speicher fällt zurück
...
Die Entladeregel **Minimum SoC** dieht dazu immer einen SoC von 4% zu halten, da unter 5% eine Entladung blockiert wird.
