...
Der EM24 in der Ausführung mit Ethernet kann mittels Modbus/TCP ausgelesen werden. Der Zähler kann je nach Montageort einer beliebigen Klasse zugeordnet werden.
...
Der Zähler kann je nach Montageort einer beliebigen Klasse zugeordnet werden.
...
Das Smart Energy Meter (KSEM G2) kann direkt mittels SunSpec kommunizieren. Der Zähler kann je nach Montageort einer beliebigen Klasse zugeordnet werden.
...
Die Siemens SENTRON Energiezähler vom Typ 7KM2200 (PAC2200) und 7KM3200 (PAC3200) können mittels Modbus/TCP ausgelesen werden. Der Zähler kann je nach Montageort einer beliebigen Klasse zugeordnet werden.

## SMA

### Sunny Boy 2.5 / 3.6
{manufacturer_to_bubbles("SMA", "Sunny Boy 2.5 / 3.6")}

SMA Wechselrichter unterstützen zum Teil SunSpec, so dass diese direkt ausgelesen werden können. Dazu zähler zum Beispiel die Sunny Boy Wechselrichter.
...
### HTTP API
{manufacturer_to_bubbles("Tinkerforge", "WARP API HTTP")}

Über die [HTTP API](/docs/interfaces/mqtt_http/introduction#http) können anegelegte API Stromzähler mit Daten versorgt werden.
Weitere Informationen hierzu gibt es auf der Seite [Schnittstellen](/docs/compatible_devices/interfaces#api) und in der [Meters API](/docs/interfaces/mqtt_http/api_reference/meters).

### MQTT API
{manufacturer_to_bubbles("Tinkerforge", "WARP API MQTT")}

Über die [MQTT API](/docs/interfaces/mqtt_http/introduction#mqtt) können anegelegte API Stromzähler mit Daten versorgt werden.
Weitere Informationen hierzu gibt es auf der Seite [Schnittstellen](/docs/compatible_devices/interfaces#api) und in der [Meters API](/docs/interfaces/mqtt_http/api_reference/meters).
...
Der YTL Stromzähler DEM4A mit RS485 (Modbus/RTU) Schnittstelle kann direkt am WARP Energy Manager angeschlossen und von diesem ausgelesen werden. Der Zähler kann je nach Montageort einer beliebigen Klasse zugeordnet werden.
