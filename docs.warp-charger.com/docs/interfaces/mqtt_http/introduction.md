...
```
können alle Nachrichten der Wallbox angezeigt werden. (Den Präfix `warp/AbCd` durch den konfigurierten ersetzen) Es könnte z.B. die folgende Nachricht auf [`warp/AbCd/evse/state`](api_reference/evse.mdx#evse_state) empfangen werden:
```jsx
{
  "iec61851_state": 1,
  "charger_state": 2,
...
```
Die Nachrichten des Topics [`evse/state`](api_reference/evse.mdx#evse_state) geben einen Überblick über den Zustand des Ladecontrollers. Beispielsweise liegt der erlaubte Ladestrom gerade bei 32000, also 32 Ampere.

Durch Senden der Nachricht `{"current":8000}` an das Topic  [`evse/global_current_update`](api_reference/evse.mdx#evse_global_current_update) kann der Ladestrom auf 8 Ampere begrenzt werden, zum Beispiel so:
```bash
...
Die HTTP-API ist strukturell identisch zur MQTT-API: Wenn die MQTT-API beispielsweise das Topic [`warp/AbCd/evse/state`](api_reference/evse.mdx#evse_state) verwendet, kann die selbe API über die URL `http://warp-AbCd/evse/state erreicht werden`. Die HTTP-API verfügt allerdings über einige fortgeschrittene Funktionen, die nicht über MQTT verfügbar sind. Es können zusätzlich die [WebSockets](https://en.wikipedia.org/wiki/WebSocket) unter z.B. `ws://warp-AbCd/ws` verwendet werden. Über eine WebSocket-Verbindung überträgt die Wallbox (analog zu MQTT) automatisch aktualisierte Werte.
...
Ein konkreteres Beispiel: Die API [`meters/1/config`](api_reference/meters.mdx#meters_X_config) konfiguriert den Stromzähler im Slot 1. Eine Wallbox im Auslieferungszustand hat nur einen Stromzähler auf Slot 0 (den möglicherweise intern verbauten Zähler) konfiguriert. Deshalb hat `meters/1/config` standardmäßig den Wert
```jsx
[0, null]
```
also das Tag 0 mit keinen weiteren Daten (ausgedrückt durch `null`)
Laut [Dokumentation dieser API](api_reference/meters.mdx#meters_X_config) bedeutet das Tag 0, dass kein Stromzähler konfiguriert ist.
