...
This documentation describes the WARP Energy Manager 2.0. The documentation for the older
generation [WARP Energy Manager 1.0](/docs/wem1/introduction) can be opened at any time
using the device selector in the menu.
...
![image](/img/wem2/wem2_face_side.jpg)

The WARP Energy Manager can control heat pumps via the SG-Ready interface depending on dynamic tariffs,
...
The WARP Energy Manager displays the measured values of the energy meters in its web interface. It shows
...
from PV inverters and battery storage, or the dynamic tariff. Power and other measured values are displayed
...
the value of the dynamic tariff, etc. can be displayed. The display turns off after 5 minutes of inactivity.
...
For PV excess utilization, the WARP Energy Manager requires an energy meter at your grid connection to determine the excess, i.e., the feed-in of
electrical power into the grid. The WARP Energy Manager then controls the devices so that no power is fed into the grid
(grid consumption = 0) or a defined grid consumption is maintained. This depends on the settings. See the [list of compatible energy meters](/compatible_devices/introduction.md).
