...
The [WARP3 Charger](/warp_charger/introduction.md) supports charge authorization via NFC. When this is enabled, an NFC tag assigned to a user must be held to the right side of the wallbox to start and/or stop a
...
A restart of the wallbox empties the list. If multiple tags are to be added one after another, we recommend holding the tags one after another to the wallbox.
...
the settings can be saved and the wallbox restarted.
