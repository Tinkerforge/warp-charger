...
via the API. For normal charge management operation, the watchdog
is not necessary.
...
The default available current is the current that the charge management
...
With this information, charge management can distribute current more efficiently:
...
in the charge management cluster.

To allow a vehicle to request more current, charge management must not limit
a charger exactly to the actual power consumption (the maximum phase current),
but must allow a certain margin so that the vehicle and charge manager can readjust.

For WARP Charger Smart, these settings are not relevant; the charge manager
