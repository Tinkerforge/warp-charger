...
- [`evse/start_charging`](api_reference/evse.mdx#evse_start_charging) starts a charging process. A charging process starts automatically unless manual charging authorization (under Wallbox -> Settings or [`evse/auto_start_charging`](api_reference/evse.mdx#evse_auto_start_charging)) is enabled.
- [`evse/stop_charging`](api_reference/evse.mdx#evse_stop_charging) stops a charging process. A stopped charging process can be reactivated with [`evse/start_charging`](api_reference/evse.mdx#evse_start_charging).
- [`evse/external_current`](api_reference/evse.mdx#evse_external_current) sets a charging current limit. For this, external control (under Wallbox -> Settings or [`evse/external_enabled`](api_reference/evse.mdx#evse_external_enabled)) must be enabled.
...
A charging current limit can also be set via [`evse/global_current`](api_reference/evse.mdx#evse_global_current).
However, this limit is stored in the flash memory of the charge controller.
When frequently setting the charging current limit, [`evse/external_current`](api_reference/evse.mdx#evse_external_current) should be used (to protect the flash memory of the charge controller).
...
- [`nfc/inject_tag_start`](api_reference/nfc.mdx#nfc_inject_tag_start) Simulates an NFC tag. The tag can start a charging process if it is known and assigned to a user. If a charging process is already running, this tag is ignored.
- [`nfc/inject_tag_stop`](api_reference/nfc.mdx#nfc_inject_tag_stop) Simulates an NFC tag. The tag can stop a charging process if it is known and assigned to a user. If no charging process is running, this tag is ignored.
- [`nfc/inject_tag`](api_reference/nfc.mdx#nfc_inject_tag) Simulates an NFC tag that can both start and stop a charging process if it is known and assigned to a user.
...
For an energy meter to be used for PV excess charging, it must measure the active power at the grid connection.
...
1. Create a new energy meter on the energy meter subpage
   ![Energy meter subpage](/img/mqtt/api_meter_1.png)
2. Enter energy meter class `API` and display name, then add value
   ![Energy meter subpage](/img/mqtt/api_meter_2.png)
3. Select the active power (sum of all phases) consumption minus feed-in as value
   ![Energy meter subpage](/img/mqtt/api_meter_3.png)
