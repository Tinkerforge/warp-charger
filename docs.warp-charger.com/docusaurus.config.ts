...
const config: Config = {
  ...
  themeConfig: {
    algolia: {
      ...
      // The stable alias paths (/docs/warp_charger, /docs/warp_energy_manager,
      // /docs/warp4_charger) are served as static client-side redirect pages, not
      // as SPA routes. A search result pointing at one of them must therefore be
      // opened with a full page load (window.location) so the redirect HTML runs;
      // a normal history.push would render "page not found". Matching these paths
      // here makes DocSearch navigate to them via window.location instead.
      externalUrlRegex: '/docs/(warp_charger|warp_energy_manager|warp4_charger)/',
